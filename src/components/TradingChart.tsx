...
export default function TradingChart() {
  ...
  function onCanvasPointerDown(e: React.PointerEvent<HTMLCanvasElement>) {
    ...
    if (!drawMode) {
      ...
      // If a drawing is already selected, check whether the touch landed on
      // one of its endpoint handles (or its body) so it can be dragged.
      if (selectedId) {
      ...
      // Otherwise: normal selection hit-test. Touch has no right-click, so a
  ...
  function onCanvasPointerUp(e: React.PointerEvent<HTMLCanvasElement>) {
    ...
    // NOTE: we intentionally do NOT force selectionMode on here. That mode
    // makes the drawing canvas capture every touch on the chart (so
    // handles can be dragged), which — if left on permanently — blocks
    // normal chart panning/zooming entirely. Tap the Cursor tool to enter
    // selection mode when you actually want to adjust a drawing.
    setDrawMode(null);
