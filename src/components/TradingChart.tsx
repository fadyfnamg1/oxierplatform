...
export default function TradingChart() {
  ...
  async function loadData(chart: any, mainSeries: any, hasSubInd: boolean, buildId: number) {
    ...
    try { mainSeries.setData(withFutureWhitespace(data, tfSecForPad)); } catch {}
  ...
  function onCanvasPointerMove(e: React.PointerEvent<HTMLCanvasElement>) {
    ...
    if (dragStateRef.current) {
      ...
      setDrawings(prev => prev.map(d => {
        ...
        if (drag.mode === 'p1') return { ...d, p1: dp };
        if (drag.mode === 'p2') return { ...d, p2: dp };
  ...
  useEffect(() => {
    ...
    function handleDown(e: PointerEvent) {
      ...
      // LWC's own canvas. Without this, LWC's internal pointerdown handler
      // (attached directly on its canvas element) already fires and starts
      // its own pan/drag the instant the pointer lands — since our overlay
      // is pointer-events:none at that moment, the browser hit-tests straight
      // through to the chart underneath. By the time we flip pointerEvents
      // back to 'all' below (in the bubble phase), LWC has already grabbed
      // the gesture, so both the drawing AND the chart end up dragging
      // together. stopPropagation() here prevents LWC's handler from ever
      // running for this pointerdown.
      e.preventDefault();
      e.stopPropagation();
      const canvas = canvasRef.current;
      if (canvas) {
